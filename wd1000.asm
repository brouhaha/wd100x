...
main_loop:
	nzt     rd2[4],main_loop	; look until CSAC
...
data_xfer_done:
	...
	; was a read command, so it's now done
        ...
        xmit    10h,aux
        and     r11,aux
        nzt     rd_ram[4:3],$+2		; check DAM mode bit
...
; Note that host could write past end of buffer
host_wr_tf_data:
...
bad_block:
	xmit    80h,r6		; set back block detect error

x0061:  xmit    0efh,command_byte & 0ffh
        xmit    2h,aux
        xor     rd_ram[6:4],aux
        move    rd_ram,r1
        nzt     aux,x0068
...
do_read_write_retry:
	xmit    3h,r11		; seek: call seek, r11 specifies return loc
        jmp     seek
...
id_field_crc_error:
	xmit    0dfh,aux	; turn on 20h bit of status reg for ID CRC err
        and     r6,r6
        xmit    20h,aux
        xor     r6,r6
        jmp     x0086
...
read_sector_crc_error:
	xmit    0bfh,aux	; turn on 40h bit in status reg for data CRC err
        and     r6,r6
        xmit    40h,aux
        xor     r6,r6
        jmp     x0086
...
read_write_too_many_index:
	xmit    40h,aux		; already have data field CRC error?
        ...
        xmit    10h,aux		; write 10h to status reg for ID not found
x012f:  move    aux,r6
        jmp     x0061
...
; write 256 bytes of 0 for data
x0182:  nzt     rd2[6],$		; wait for bdone
        xmit    0h,wr_serdes
...
; save task file registers for scratch use
        ...
        xmit    drive_3_cylinder_high,aux	; get drive's current cylinder in r2 (hi), r1 (low)
        and     r5,ram_addr_low
        move    rd_ram,r2
        move    rd_ram,r1
...
; get step rate in r2
	...
	add     r4,r4
        ...
        move    r5,drive_control	; turn off setp bit
